...
#        path: path to the first numpy array taken in which the batch will be taken
    ...
    def __getitem__(self, idx):
        ...
        y[:,0] = np.array(new_data['e1_0'][indices])#np.exp(np.array(new_data['e1'][indices]))*2/(np.max(np.exp(np.array(new_data['e1'][indices]))*2))#np.exp(np.array(new_data['e1'][indices]))*2 # np.array(new_data['e1'][indices])
        y[:,1] = np.array(new_data['e2_0'][indices])#np.exp(np.array(new_data['e2'][indices]))*2/(np.max(np.exp(np.array(new_data['e2'][indices]))*2))#np.exp(np.array(new_data['e2'][indices]))*2 # np.array(new_data['e2'][indices]) # 
        y[:,2] = np.array(new_data['redshift_0'][indices])#/(np.max(np.array(new_data['redshift'][indices])))
