...
I_lsst = tf.convert_to_tensor(np.array([255.2383, 2048.9297, 3616.1757, 4441.0576, 4432.7823, 2864.145]))
I_euclid = tf.convert_to_tensor(np.array([5925.8097, 3883.7892, 1974.2465,  413.3895]))
beta = 2.5
...
def norm(x, bands,n_years, channel_last=False, inplace=True):
    I = tf.concat([I_euclid,n_years*I_lsst])
    if not inplace:
        y = tf.identity(x)
    else:
        y = x
    if channel_last:
        ...
        for i in range (len(y)):
            for ib, b in enumerate(bands):
                y[i,:,:,ib] = tf.math.tanh(tf.math.arcsinh(y[i,:,:,ib]/(I[b]/beta)))
    else:
        assert y.shape[1] == len(bands)
        for i in range (len(y)):
            for ib, b in enumerate(bands):
                y[i,ib] = tf.math.tanh(np.arcsinh(y[i,ib]/(I[b]/beta)))
    return y
...
def denorm(x, bands,n_years, channel_last=False, inplace=True):
    ...
    if channel_last:
        ...
        for i in range (len(y)):
            for ib, b in enumerate(bands):
                y[i,:,:,ib] = tf.math.sinh(tf.math.arctanh(y[i,:,:,ib]))*(I[b]/beta)
    else:
        assert y.shape[1] == len(bands)
        for i in range (len(y)):
            for ib, b in enumerate(bands):
                y[i,ib] = tf.math.sinh(tf.math.arctanh(x[i,ib]))*(I[b]/beta)
    return y
...
def SNR_peak(gal_noiseless, sky_background_pixel, band=6, snr_min=2):
    ...
    snr = tf.math.maximum(gal_noiseless[band])/sky_background_pixel[band]
...
def SNR(gal_noiseless, sky_background_pixel, band=6, snr_min=5):
    ...
    snr = tf.math.sqrt(np.sum(signal**2/variance))
    return (snr>snr_min), snr
...
def compute_blendedness_single(image1, image2):
    ...
    if isinstance(image1, galsim.image.Image):
        im1 = tf.Tensor(image1.array.data)
        im2 = tf.Tensor(image2.array.data)
    else:
        im1 = image1
        im2 = image2
    # print(image,image_new)
    blnd = tf.math.reduce_sum(im1*im2)/np.sqrt(np.sum(im1**2)*np.sum(im2**2))
    return blnd

def compute_blendedness_total(img_central, img_others):
    if isinstance(img_central, galsim.image.Image):
        ic = tf.Tensor(img_central.array.data)
        io = tf.Tensor(img_others.array.data)
    else :
        ic = img_central
        io = img_others
    itot = ic + io
    ...
    blnd = 1. - tf.math.reduce_sum(ic*ic)/tf.math.reduce_sum(itot*ic)
    return blnd
...
def compute_blendedness_aperture(img_central, img_others, radius):
    ...
    flux_central = tf.math.reduce_sum(ic*mask.astype(float))
    flux_others = tf.math.reduce_sum(io*mask.astype(float))
...
def compute_deltas_for_most_blended(shift,mag,blendedness):#(shift_path, mag_path):
    ...
    mag_min = tf.zeros(len(mag))
    blend_max = tf.zeros(len(blendedness))
    for k in range (len(mag)):
        mag_min[k] = tf.math.reduce_min(mag[k])
        if (len(blendedness[k])>=1):
            blend_max[k] = tf.math.reduce_max(blendedness[k])
        else:
            blend_max[k] = 0

    # set lists
    deltas_r= tf.zeros((len(shift),3))
    delta_r= tf.zeros((len(shift)))
    delta_mag = tf.zeros((len(shift)))
    deltas_mag= tf.zeros((len(shift),4))

    # TODO avoid the loops?
    for i in range (len(shift)):
        for j in range (len(shift[i])):
            deltas_r[i][j] = tf.math.sqrt(np.square(shift[i][j][0])+tf.math.square(shift[i][j][1]))
    ...
    deltas_mag_3= tf.zeros((len(deltas_mag),3))
    counter = 0
...
def delta_r_min(shift_path):
    ...
    # set lists
    deltas_r= tf.zeros((len(shift),4))
    delta_r= tf.zeros((len(shift)))
    
    # compute the delta r for each couple of galaxies
    for i in range (4):
        deltas_r[:,i] = tf.math.sqrt(tf.math.square(shift[:,i,0])+tf.math.square(shift[:,i,1]))
