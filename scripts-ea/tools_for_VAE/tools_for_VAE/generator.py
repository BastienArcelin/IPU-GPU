...
#        path: path to the first numpy array taken in which the batch will be taken
        ...
        """
        self.bands = tf.convert_to_tensor(bands)
        ...
        self.p = tf.convert_to_tensor(np.array(self.p))
        self.total_sample_size = int(tf.math.reduce_sum(self.p))
        ...
        self.p /= tf.math.reduce_sum(self.p)
    ...
    def __getitem__(self, idx):
        ...
        # todo convert this to tf ?
        y = np.zeros((self.batch_size, 3))
        y[:,0] = new_data['e1_0'][indices]#np.exp(np.array(new_data['e1'][indices]))*2/(np.max(np.exp(np.array(new_data['e1'][indices]))*2))#np.exp(np.array(new_data['e1'][indices]))*2 # np.array(new_data['e1'][indices])
        y[:,1] = new_data['e2_0'][indices]#np.exp(np.array(new_data['e2'][indices]))*2/(np.max(np.exp(np.array(new_data['e2'][indices]))*2))#np.exp(np.array(new_data['e2'][indices]))*2 # np.array(new_data['e2'][indices]) # 
        y[:,2] = new_data['redshift_0'][indices]#/(np.max(np.array(new_data['redshift'][indices])))
