...
load = str(sys.argv[3]).lower() == 'true'
#lr = sys.argv[4]
bands = [4,5,6,7,8,9]
...
# , amsgrad = True
######## Callabcks
path_weights = '/sps/lsst/users/barcelin/TFP/weights/shapes/'
checkpointer_loss = ModelCheckpoint(filepath=path_weights+'test3/weights_loss_noisy_v4.{epoch:02d}-{val_loss:.2f}.ckpt', 
                                    monitor='val_loss', 
                                    verbose=1, 
                                    save_best_only=True, 
...
callbacks = [ checkpointer_loss]#changelr(model) checkpointer_loss
